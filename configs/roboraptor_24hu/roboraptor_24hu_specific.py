...
def get_meta_from_articles_spec(tei_logger, url, bs):
    ...
    data['sch:url'] = url
    if bs.find('meta', content='activity'):
        date_tag = bs.find('div', class_='m-author__wrapCatDateTitulus').get_text()
        if date_tag is not None:
            parsed_date = parse_date(date_tag[-26:-7].replace(' ', ''), '%Y-%m-%dT%H:%M:%S')
